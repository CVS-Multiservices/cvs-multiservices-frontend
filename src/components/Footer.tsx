...
// ── Social link config (map DB field → icon + color) ──────────────────────────
const socialConfig: {
  key: string;
  label: string;
...
export default function Footer() {
  ...
  const [emailAddress, setEmailAddress] = useState('info@cvsmultiservices.com');
  const [phoneNumber, setPhoneNumber] = useState('+91 97235 51751');
  ...
  const [activeSocials, setActiveSocials] = useState<
    { key: string; label: string; icon: React.FC<{ className?: string; style?: CSSProperties }>; color: string; url: string }[]
  >([]);
    ...
    const fetchFooterData = async () => {
      try {
        ...
        if (linksRes.success && linksRes.data && linksRes.data.length > 0) {
          ...
          // Build active socials array — only include non-null URLs
        ...
        // ── Process Contact ──
        if (contactRes.success && contactRes.data && contactRes.data.length > 0) {
          const contactData = contactRes.data[0] as any;
          const contactInfo = contactData?.contactInfo;

          if (Array.isArray(contactInfo)) {
            // Find address entry (icon === "MapPin" or title contains "Address")
            const addressEntry = contactInfo.find(
              (c: any) => c.icon === 'MapPin' || c.title?.toLowerCase().includes('address')
            );
            if (addressEntry?.lines && Array.isArray(addressEntry.lines)) {
              setAddressLines(addressEntry.lines);
            }

            // Find phone entry (icon === "Phone" or title contains "Phone")
            const phoneEntry = contactInfo.find(
              (c: any) => c.icon === 'Phone' || c.title?.toLowerCase().includes('phone')
            );
            if (phoneEntry?.lines && phoneEntry.lines.length > 0) {
              setPhoneNumber(phoneEntry.lines[0]); // first number
            }

            // Find email entry (icon === "Mail" or title contains "Email")
            const emailEntry = contactInfo.find(
              (c: any) => c.icon === 'Mail' || c.title?.toLowerCase().includes('email')
            );
            if (emailEntry?.lines && emailEntry.lines.length > 0) {
              setEmailAddress(emailEntry.lines[0]); // first email
            }
          }
        }
      } catch (err) {
      ...
      <div className="w-full px-4 sm:px-8 lg:px-12 xl:px-16 2xl:px-24 py-14 lg:py-20 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 xl:gap-10 2xl:gap-12">
          ...
          <div className="lg:col-span-5 xl:col-span-5 2xl:col-span-5">
            ...
            >
              India's leading industrial services company specialising in Effluent Treatment and Waste Management since 2017.
            </p>

            {/* Contact info from API */}
            <div className="space-y-3">
              {/* Address — all 3 lines joined */}
              <div className="space-y-3">
                {/* Address */}
                <div className="flex items-start gap-2.5 group cursor-pointer transition-all duration-300 hover:translate-x-1">
                  ...
                  >
                    ...
                    {bottomAddress && <span className="block">{bottomAddress}</span>}
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-2.5">
              {quickLinks.map((link) => (
                <Link
                  ...
                  onMouseLeave={(e) => {
                    (e.currentTarget as HTMLElement).style.color = COLORS.textHalf;
                  }}
                >
                  <span
          ...
          {/* ── Social media (from API — only non-null) ── */}
